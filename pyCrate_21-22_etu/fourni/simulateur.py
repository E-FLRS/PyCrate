...
SCORE_DATE_FILE_PATH: str = os.path.abspath("./scores/date_scores.txt")
...
def load_scores(jeu: Jeu):
    ...
    pyCrate.chargement_date_score(SCORE_DATE_FILE_PATH, jeu.dict_date_score)


def refresh_score(jeu: Jeu):
    score_affichage = pyCrate.maj_score(jeu.niveau_en_cours, jeu.dict_scores, jeu.dict_date_score)
    jeu.score_label.config(text=score_affichage)


def save_score(jeu: Jeu):
    pyCrate.enregistre_score(jeu.temps_initial, jeu.nb_coups, SCORE_BASE, jeu.dict_scores, jeu.niveau_en_cours,jeu.dict_date_score)
    refresh_score(jeu)


def update_score_file(jeu: Jeu, fenetre: Tk):
    pyCrate.update_score_file(SCORE_FILE_PATH, jeu.dict_scores, SCORE_DATE_FILE_PATH, jeu.dict_date_score)
    quitter(fenetre)
...
def simulate():
    ...
    can: Canvas = Canvas(fenetre, height=760, width=1000, bg="#80D0D0")
    can.pack(side=LEFT)
    jeu: Jeu = Jeu(can)
    ...
    score_label: Label = Label(fenetre, anchor="nw", font="Cooper", justify="left", bg="#1B019B", fg="white",
    ...
    t: Label = Label(fenetre, anchor="s", font="Cooper", bg="#1B019B", fg="white", pady="20",
                     text=_help,
                     height=18, width=38)
    t.pack(side=LEFT)
